...
let profile={email:'maria@univ.edu.ph',contact:'09171234567'};
...
function handleLogout(){openModal(`<div class="text-center p-2"><div class="w-12 h-12 rounded-full bg-green-100 text-green-700 flex items-center justify-center mx-auto mb-3"><i class="fas fa-sign-out-alt"></i></div><h3 class="font-bold text-lg mb-2">Log out?</h3><p class="text-sm text-gray-500 mb-5">Are you sure you want to log out of your account?</p><div class="flex gap-2 justify-center"><button type="button" class="btn-secondary" onclick="closeModal()">Cancel</button><button type="button" class="btn-danger" onclick="confirmLogout()">Log out</button></div></div>`)}
function confirmLogout(){closeModal();sessionStorage.removeItem('drrts_role');currentRole='';currentPage='dashboard';document.getElementById('app-container').style.display='none';document.getElementById('login-screen').style.display='flex';document.getElementById('login-user').value='';document.getElementById('login-pass').value=''}
...
function buildNav(){
const adminNav=[{page:'dashboard',icon:'fa-tachometer-alt',label:'Dashboard'},{page:'requests',icon:'fa-file-alt',label:'Requests'},{page:'students',icon:'fa-users',label:'Students'},{page:'payments',icon:'fa-credit-card',label:'Payments'},{page:'schedules',icon:'fa-calendar-alt',label:'QR Schedules'},{page:'doctypes',icon:'fa-folder-open',label:'Document Types'},{page:'notifications',icon:'fa-bell',label:'Notifications'},{page:'reports',icon:'fa-chart-bar',label:'Reports'},{page:'profile',icon:'fa-user',label:'Profile'}];
const studentNav=[{page:'dashboard',icon:'fa-tachometer-alt',label:'Dashboard'},{page:'requests',icon:'fa-file-alt',label:'My Requests'},{page:'payments',icon:'fa-credit-card',label:'Payments'},{page:'schedules',icon:'fa-calendar-alt',label:'Pick-up Schedule'},{page:'notifications',icon:'fa-bell',label:'Notifications'},{page:'doctypes',icon:'fa-folder-open',label:'Document Types'},{page:'about',icon:'fa-sitemap',label:'About Us'},{page:'profile',icon:'fa-user',label:'Profile'}];
...
function requestConfirmation(title,message,action){pendingAction=action;openModal(`<div class="text-center p-2"><div class="w-12 h-12 rounded-full bg-green-100 text-green-700 flex items-center justify-center mx-auto mb-3"><i class="fas fa-circle-question"></i></div><h3 class="font-bold text-lg mb-2">${title}</h3><p class="text-sm text-gray-500 mb-5">${message}</p><div class="flex gap-2 justify-center"><button type="button" class="btn-secondary" onclick="pendingAction=null;closeModal()">Cancel</button><button type="button" class="btn-success" onclick="runConfirmedAction()">Continue</button></div></div>`)}
function runConfirmedAction(){const action=pendingAction;pendingAction=null;closeModal();if(action)action()}
...
// ===== RENDER =====
function renderPage(){const c=document.getElementById('content-area');const pages={dashboard:currentRole==='admin'?adminDash:studentDash,requests:requestsPage,payments:paymentsPage,schedules:schedulesPage,students:studentsPage,doctypes:docTypesPage,notifications:notificationsPage,reports:reportsPage,profile:profilePage,about:aboutPage};c.innerHTML=(pages[currentPage]||pages.dashboard)();updateNotifBadge();if(currentPage==='dashboard'&&currentRole==='admin')setTimeout(initCharts,60)}
...
<div class="bg-white rounded-xl p-5 shadow-sm"><h3 class="font-semibold text-sm mb-3">Requested Documents</h3><canvas id="chartRevenue" height="160"></canvas></div>
</div>
...
function confirmCSVImport(confirmed=false){
if(!csvPendingRows.length)return;
if(!confirmed){requestConfirmation('Import students?','This will add '+csvPendingRows.length+' students to the records.',()=>confirmCSVImport(true));return}
...
function paymentsPage(){const all=currentRole==='student'?payments.filter(p=>p.student==='Maria Santos'):payments;const filtered=paymentFilter==='All'?all:all.filter(p=>p.status===paymentFilter);
return `<div class="bg-white rounded-xl p-5 shadow-sm"><div class="flex flex-wrap items-center justify-between gap-3 mb-4"><h3 class="font-semibold text-sm">Payments</h3><select class="border rounded-lg px-2 py-1.5 text-xs" onchange="paymentFilter=this.value;renderPage()"><option>All</option><option ${paymentFilter==='Verified'?'selected':''}>Verified</option><option ${paymentFilter==='Payment Pending'?'selected':''}>Payment Pending</option><option ${paymentFilter==='Pending Verification'?'selected':''}>Pending Verification</option></select></div><div class="overflow-x-auto"><table><thead><tr><th>ID</th><th>Request</th>${currentRole==='admin'?'<th>Student</th>':''}<th>Amount</th><th>Reference</th><th>Status</th>${currentRole==='admin'?'<th>Action</th>':''}</tr></thead><tbody>${filtered.map(p=>`<tr><td>${p.id}</td><td class="text-blue-600 cursor-pointer" onclick="showRequestDetail('${p.request}')">${p.request}</td>${currentRole==='admin'?`<td>${sanitize(p.student)}</td>`:''}<td>₱${p.amount}</td><td>${sanitize(p.ref)||'—'}</td><td>${badge(p.status)}</td>${currentRole==='admin'?`<td>${p.status==='Pending Verification'?`<button onclick="verifyPayment('${p.id}')" class="text-green-600 text-xs font-medium"><i class="fas fa-check-circle"></i> Verify</button>`:''}</td>`:''}</tr>`).join('')||'<tr><td colspan="7" class="empty-state">No payments found</td></tr>'}</tbody></table></div></div>`}

function schedulesPage(){const filtered=currentRole==='student'?schedules.filter(s=>s.student==='Maria Santos'):schedules;
return `<div class="bg-white rounded-xl p-5 shadow-sm"><div class="flex items-center justify-between mb-4"><h3 class="font-semibold text-sm">Pick-up Schedule</h3>${currentRole==='admin'?`<button onclick="openAssignSchedule()" class="btn-primary"><i class="fas fa-qrcode mr-1"></i>Pick Schedule</button>`:''}</div><table><thead><tr><th>Request</th>${currentRole==='admin'?'<th>Student</th>':''}<th>Document</th><th>Date</th><th>Time</th><th>Status</th><th>Action</th></tr></thead><tbody>${filtered.length?filtered.map(s=>`<tr><td class="text-blue-600 cursor-pointer" onclick="showRequestDetail('${s.request}')">${s.request}</td>${currentRole==='admin'?`<td>${sanitize(s.student)}</td>`:''}<td>${sanitize(s.doc)}</td><td>${s.date}</td><td>${s.time}</td><td>${badge(s.status)}</td><td><button onclick="generateQR('${s.request}')" class="text-green-600 text-xs" title="Generate QR Code"><i class="fas fa-qrcode"></i></button></td></tr>`).join(''):'<tr><td colspan="7" class="empty-state">No schedules yet</td></tr>'}</tbody></table></div>`}
...
function notificationsPage(){return `<div class="bg-white rounded-xl p-5 shadow-sm"><div class="flex items-center justify-between mb-4"><h3 class="font-semibold text-sm">Notifications <span class="text-gray-400 font-normal">(${notifications.filter(n=>!n.read).length} unread)</span></h3><button onclick="markAllRead()" class="text-blue-600 text-xs font-medium hover:underline">Mark all read</button></div><div class="space-y-2">${notifications.map(n=>`<div class="flex items-start gap-3 p-3 rounded-lg border ${n.read?'border-gray-100 bg-white':'border-blue-200 bg-blue-50'}"><button class="w-7 h-7 rounded-full ${n.read?'bg-gray-100':'bg-blue-100'} flex items-center justify-center flex-shrink-0" onclick="toggleNotifRead(${n.id})" title="Open notification"><i class="fas fa-bell text-[11px] ${n.read?'text-gray-400':'text-blue-600'}"></i></button><div class="flex-1 cursor-pointer" onclick="toggleNotifRead(${n.id})"><div class="text-xs">${sanitize(n.msg)}</div><div class="text-[11px] text-gray-400 mt-0.5">${n.time}</div></div><button onclick="deleteNotification(${n.id})" class="text-red-500 text-xs" title="Delete notification"><i class="fas fa-trash"></i></button>${!n.read?'<span class="w-2 h-2 bg-blue-600 rounded-full mt-2"></span>':''}</div>`).join('')||'<div class="empty-state">No notifications</div>'}</div></div>`}

function profilePage(){return `<div class="bg-white rounded-xl p-5 shadow-sm max-w-2xl"><h3 class="font-semibold text-sm mb-4">Profile</h3><div class="grid sm:grid-cols-2 gap-3"><label class="text-xs text-gray-600">Email<input id="profile-email" class="border rounded-lg px-3 py-2 text-sm w-full mt-1" value="${sanitize(profile.email)}" type="email"></label><label class="text-xs text-gray-600">Contact number<input id="profile-contact" class="border rounded-lg px-3 py-2 text-sm w-full mt-1" value="${sanitize(profile.contact)}"></label></div><div class="flex flex-wrap gap-2 mt-4"><button onclick="saveProfile()" class="btn-success">Save profile</button><button onclick="openChangePassword()" class="btn-secondary">Change password</button><button onclick="openChangeEmail()" class="btn-secondary">Change email</button></div></div>`}
function aboutPage(){return `<div class="bg-white rounded-xl p-5 shadow-sm"><h3 class="font-semibold text-sm mb-4">Organizational Chart</h3><div class="org-chart"><div class="org-node org-lead">Registrar</div><div class="org-line"></div><div class="org-branches"><div class="org-node">Records Officer</div><div class="org-node">Finance Officer</div><div class="org-node">Releasing Officer</div></div></div></div>`}
...
// ===== ACTIONS =====
function approveRequest(id,confirmed=false){const r=requests.find(x=>x.id===id);if(!r||r.status!=='Pending')return;if(!confirmed){requestConfirmation('Approve request?','This will approve '+id+'.',()=>approveRequest(id,true));return}r.status='Approved';r.history.push({s:'Approved',t:now(),by:'Admin'});addLog('Approved request '+id,'approve');addNotif('Request '+id+' approved');showToast('Request '+id+' approved');renderPage()}
function cancelRequest(id,confirmed=false){const r=requests.find(x=>x.id===id);if(!r||r.status!=='Pending')return;if(!confirmed){requestConfirmation('Cancel request?','This will cancel '+id+'.',()=>cancelRequest(id,true));return}r.status='Cancelled';r.history.push({s:'Cancelled',t:now(),by:'Admin'});addLog('Cancelled request '+id,'cancel');addNotif('Request '+id+' cancelled');showToast('Request '+id+' cancelled','error');renderPage()}
function processRequest(id,confirmed=false){const r=requests.find(x=>x.id===id);if(!r||r.status!=='Approved'||!r.paid)return;if(!confirmed){requestConfirmation('Process request?','This will move '+id+' into processing.',()=>processRequest(id,true));return}r.status='Processing';r.history.push({s:'Processing',t:now(),by:'Admin'});addLog('Processing '+id,'process');addNotif('Request '+id+' is now processing');showToast('Processing '+id,'info');renderPage()}
function readyRequest(id,confirmed=false){const r=requests.find(x=>x.id===id);if(!r||r.status!=='Processing')return;if(!confirmed){requestConfirmation('Mark request ready?','This will mark '+id+' ready for pickup.',()=>readyRequest(id,true));return}r.status='Ready for Pickup';r.history.push({s:'Ready for Pickup',t:now(),by:'Admin'});addLog('Marked '+id+' ready','ready');addNotif('Request '+id+' ready for pickup');showToast(id+' ready');renderPage()}
function releaseRequest(id,confirmed=false){const r=requests.find(x=>x.id===id);if(!r||r.status!=='Ready for Pickup')return;if(!confirmed){requestConfirmation('Release request?','This will release '+id+' to the student.',()=>releaseRequest(id,true));return}r.status='Released';r.history.push({s:'Released',t:now(),by:'Admin'});const sched=schedules.find(s=>s.request===id);if(sched)sched.status='Released';addLog('Released '+id,'release');addNotif(id+' released to '+r.student);showToast(id+' released');renderPage()}
function verifyPayment(id,confirmed=false){const p=payments.find(x=>x.id===id);if(!p||p.status==='Verified')return;if(!confirmed){requestConfirmation('Verify payment?','This will verify payment '+id+'.',()=>verifyPayment(id,true));return}p.status='Verified';p.date=now().slice(0,10);const r=requests.find(x=>x.id===p.request);if(r){r.paid=true;r.history.push({s:'Payment Verified',t:now(),by:'Admin'})}addLog('Verified payment '+id,'verify');addNotif('Payment verified for '+p.request);showToast('Payment verified');renderPage()}
function markAllRead(confirmed=false){if(!confirmed){requestConfirmation('Mark notifications read?','This will mark all notifications as read.',()=>markAllRead(true));return}notifications.forEach(n=>n.read=true);updateNotifBadge();renderPage()}
function toggleNotifRead(id){const n=notifications.find(x=>x.id===id);if(n){n.read=!n.read;updateNotifBadge();renderPage()}}
function deleteNotification(id,confirmed=false){if(!confirmed){requestConfirmation('Delete notification?','This notification will be permanently removed.',()=>deleteNotification(id,true));return}notifications=notifications.filter(n=>n.id!==id);updateNotifBadge();showToast('Notification deleted','info');renderPage()}
function generateQR(requestId){const s=schedules.find(x=>x.request===requestId);if(!s)return;const payload=encodeURIComponent(`${s.request}|${s.doc}|${s.date}|${s.time}`);openModal(`<div class="flex items-center justify-between mb-4"><h3 class="font-bold">Pickup QR Code</h3><button onclick="closeModal()" class="text-gray-400 text-lg">&times;</button></div><div class="text-center"><img class="qr-image" src="https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${payload}" alt="QR code for ${sanitize(s.request)}"><p class="text-sm font-medium mt-3">${sanitize(s.request)}</p><p class="text-xs text-gray-500">${sanitize(s.date)} at ${sanitize(s.time)}</p><a class="btn-success inline-block mt-4" download="${s.request}-qr.png" href="https://api.qrserver.com/v1/create-qr-code/?size=600x600&data=${payload}"><i class="fas fa-download mr-1"></i>Download QR</a></div>`)}
function saveProfile(confirmed=false,values=null){const email=values?values.email:document.getElementById('profile-email').value.trim();const contact=values?values.contact:document.getElementById('profile-contact').value.trim();if(!email||!contact){showToast('Email and contact number are required','error');return}if(!confirmed){requestConfirmation('Save profile changes?','Your email and contact information will be updated.',()=>saveProfile(true,{email,contact}));return}profile={email,contact};showToast('Profile updated');renderPage()}
function openChangePassword(){openModal(`<h3 class="font-bold mb-4">Change Password</h3><form onsubmit="event.preventDefault();confirmCredentialChange('Password')"><input id="credential-value" type="password" class="border rounded-lg px-3 py-2 text-sm w-full" placeholder="New password" required><input id="credential-confirm" type="password" class="border rounded-lg px-3 py-2 text-sm w-full mt-2" placeholder="Confirm new password" required><div class="flex gap-2 mt-4"><button class="btn-success flex-1">Change password</button><button type="button" onclick="closeModal()" class="btn-secondary flex-1">Cancel</button></div></form>`)}
function openChangeEmail(){openModal(`<h3 class="font-bold mb-4">Change Email</h3><form onsubmit="event.preventDefault();confirmCredentialChange('Email')"><input id="credential-value" type="email" class="border rounded-lg px-3 py-2 text-sm w-full" placeholder="New email" required><input id="credential-confirm" type="password" class="border rounded-lg px-3 py-2 text-sm w-full mt-2" placeholder="Current password" required><div class="flex gap-2 mt-4"><button class="btn-success flex-1">Change email</button><button type="button" onclick="closeModal()" class="btn-secondary flex-1">Cancel</button></div></form>`)}
function confirmCredentialChange(type,confirmed=false,value=null){const fieldValue=value===null?document.getElementById('credential-value').value:value;const confirmValue=value===null?document.getElementById('credential-confirm').value:'';if(type==='Password'&&fieldValue!==confirmValue){showToast('Passwords do not match','error');return}if(!confirmed){requestConfirmation('Confirm '+type.toLowerCase()+' change?','Your '+type.toLowerCase()+' will be updated.',()=>confirmCredentialChange(type,true,fieldValue));return}if(type==='Email')profile.email=fieldValue;closeModal();showToast(type+' updated');renderPage()}
...
<div><span class="text-gray-500 text-xs">Purpose</span><div>${sanitize(r.purpose)||'—'}</div></div>
<div><span class="text-gray-500 text-xs">Payment Mode</span><div>${sanitize(r.paymentMode||(p&&p.mode))||'—'}</div></div>
...
<div><label class="text-xs font-medium text-gray-600 block mb-1" for="nr-payment">Mode of Payment</label><select id="nr-payment" class="border rounded-lg px-3 py-2 text-sm w-full"><option>GCash</option><option>Bank Transfer</option><option>Over the Counter</option></select></div>
...
function submitRequest(confirmed=false,values=null){
const purposeEl=document.getElementById('nr-purpose');const purpose=values?values.purpose:purposeEl.value.trim();
if(!purpose){purposeEl.classList.add('field-error');document.getElementById('nr-purpose-err').classList.remove('hidden');return}
const studentVal=values?values.studentVal:document.getElementById('nr-student').value;const docId=values?values.docId:parseInt(document.getElementById('nr-doc').value);const copies=values?values.copies:Math.max(1,parseInt(document.getElementById('nr-copies').value)||1);const paymentMode=values?values.paymentMode:document.getElementById('nr-payment').value;const d=docTypes.find(x=>x.id===docId);
if(requests.some(r=>r.student===studentVal&&r.doc===d.name&&!['Released','Completed','Cancelled','Rejected'].includes(r.status))){showToast('Duplicate document request','error');return}
if(!confirmed){requestConfirmation('Submit document request?','Your request for '+d.name+' will be submitted.',()=>submitRequest(true,{purpose,studentVal,docId,copies,paymentMode}));return}
const id=`REQ-2026-${String(nextReqNum++).padStart(3,'0')}`;const fee=d.fee*copies;
requests.unshift({id,student:studentVal,doc:d.name,date:new Date().toISOString().slice(0,10),status:'Pending',fee,paid:false,purpose,copies,paymentMode,history:[{s:'Created',t:now(),by:studentVal}]});
payments.push({id:`PAY-${String(nextPayNum++).padStart(3,'0')}`,request:id,student:studentVal,amount:fee,ref:'',date:'',mode:paymentMode,status:'Payment Pending'});
...
function submitReceipt(reqId,confirmed=false,submittedRef=null){const refEl=document.getElementById('rc-ref');const ref=submittedRef===null?refEl.value.trim():submittedRef;if(!ref){if(refEl){refEl.classList.add('field-error');document.getElementById('rc-ref-err').classList.remove('hidden')}return}
if(!confirmed){requestConfirmation('Submit payment?','Your payment reference will be submitted for verification.',()=>submitReceipt(reqId,true,ref));return}
...
closeModal();showToast('Payment submitted');showReceipt(reqId)}

function showReceipt(reqId){const r=requests.find(x=>x.id===reqId);const p=getPaymentForReq(reqId);if(!r)return;const canvas=document.createElement('canvas');canvas.width=900;canvas.height=560;const ctx=canvas.getContext('2d');ctx.fillStyle='#ffffff';ctx.fillRect(0,0,900,560);ctx.fillStyle='#198754';ctx.fillRect(0,0,900,110);ctx.fillStyle='#ffffff';ctx.font='bold 30px DM Sans';ctx.fillText('COLM PAYMENT RECEIPT',42,68);ctx.fillStyle='#1f2937';ctx.font='24px DM Sans';ctx.fillText(`Request: ${r.id}`,42,175);ctx.font='20px DM Sans';ctx.fillText(`Student: ${r.student}`,42,225);ctx.fillText(`Document: ${r.doc}`,42,275);ctx.fillText(`Amount Paid: PHP ${r.fee}`,42,325);ctx.fillText(`Reference: ${p&&p.ref||'Pending'}`,42,375);ctx.fillText(`Date: ${p&&p.date||new Date().toISOString().slice(0,10)}`,42,425);const image=canvas.toDataURL('image/png');openModal(`<h3 class="font-bold mb-4">Payment Receipt</h3><img class="receipt-image" src="${image}" alt="Payment receipt for ${sanitize(r.id)}"><div class="flex gap-2 mt-4"><a class="btn-success flex-1 text-center" download="${r.id}-receipt.png" href="${image}"><i class="fas fa-download mr-1"></i>Download PNG</a><a class="btn-secondary flex-1 text-center" href="mailto:${encodeURIComponent(profile.email)}?subject=${encodeURIComponent('Payment receipt '+r.id)}&body=${encodeURIComponent('Attached receipt for '+r.id+'.')}"><i class="fas fa-envelope mr-1"></i>Email</a></div>`)}
...
function submitSchedule(confirmed=false,values=null){const reqId=values?values.reqId:document.getElementById('sc-req').value;const date=values?values.date:document.getElementById('sc-date').value;const time=values?values.time:document.getElementById('sc-time').value;
if(!date){document.getElementById('sc-date-err').classList.remove('hidden');return}
if(!confirmed){requestConfirmation('Assign pickup schedule?','This pickup schedule will be assigned to '+reqId+'.',()=>submitSchedule(true,{reqId,date,time}));return}
...
function submitDocType(confirmed=false,values=null){const nameEl=document.getElementById('dt-name');const name=values?values.name:nameEl.value.trim();if(!name){nameEl.classList.add('field-error');document.getElementById('dt-name-err').classList.remove('hidden');return}const fee=values?values.fee:Math.max(0,parseInt(document.getElementById('dt-fee').value)||0);const days=values?values.days:Math.max(1,parseInt(document.getElementById('dt-days').value)||1);const reqs=values?values.reqs:document.getElementById('dt-reqs').value.trim()||'None';
if(!confirmed){requestConfirmation('Add document type?','The document type '+name+' will be added.',()=>submitDocType(true,{name,fee,days,reqs}));return}
docTypes.push({id:docTypes.length+1,name,fee,days,reqs});
addLog('Added doc type: '+name,'create');closeModal();showToast('Added');renderPage()}

function exportCSV(confirmed=false){if(!confirmed){requestConfirmation('Export request report?','A CSV report of the current requests will be downloaded.',()=>exportCSV(true));return}let csv='ID,Student,Document,Date,Status,Fee,Paid\n';requests.forEach(r=>csv+=`"${r.id}","${r.student}","${r.doc}","${r.date}","${r.status}",${r.fee},${r.paid}\n`);const blob=new Blob([csv],{type:'text/csv'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='drrts_requests.csv';a.click();showToast('Exported')}
...
const docs={};requests.forEach(r=>{docs[r.doc]=(docs[r.doc]||0)+1});
chart2=new Chart(ctx2,{type:'bar',data:{labels:Object.keys(docs),datasets:[{label:'Requests',data:Object.values(docs),backgroundColor:'#198754',borderRadius:6}]},options:{responsive:true,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true, ticks:{precision:0}}}}});
}
